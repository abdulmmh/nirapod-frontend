...
export class DashboardHomeComponent implements OnInit, OnDestroy {
  ...
  private buildBars(): void {
    ...
    const safe   = (arr: any) => Array.isArray(arr) && arr.length > 0 ? arr : [{ value: 0 }];
    const maxOf  = (arr: any[]) => Math.max(...arr.map(d => d?.value ?? 0)) || 1;
    const pct    = (arr: any[], max: number) => arr.map(d => Math.round(((d?.value ?? 0) / max) * 100));

    const vat  = safe(this.chartData.vatChart);
    const pay  = safe(this.chartData.paymentChart);
    const aud  = safe(this.chartData.auditChart);
    const myp  = safe(this.chartData.myPaymentChart);

    this.vatBars     = pct(vat, maxOf(vat));
    this.paymentBars = pct(pay, maxOf(pay));
    this.auditBars   = pct(aud, maxOf(aud));
    this.myPayBars   = pct(myp, maxOf(myp));
  }
