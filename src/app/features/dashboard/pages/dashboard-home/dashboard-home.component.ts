...
import {
  DashboardStats, RecentTaxpayer, RecentPayment, DashboardChartData
...
export class DashboardHomeComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  // ── Stat Cards ──
  statCards = [
    { label: 'Total Taxpayers', value: 0, change: 0, icon: 'bi bi-people-fill',      color: 'teal',   progress: 0,  suffix: ''    },
    { label: 'Total Revenue',   value: 0, change: 0, icon: 'bi bi-currency-dollar',   color: 'blue',   progress: 0,  suffix: ' Cr' },
    { label: 'VAT Returns',     value: 0, change: 0, icon: 'bi bi-arrow-repeat',      color: 'purple', progress: 0,  suffix: ''    },
    { label: 'Total Payments',  value: 0, change: 0, icon: 'bi bi-credit-card-fill',  color: 'green',  progress: 0,  suffix: ''    },
    { label: 'Pending Audits',  value: 0, change: 0, icon: 'bi bi-shield-fill-check', color: 'orange', progress: 0,  suffix: ''    },
    { label: 'Pending Refunds', value: 0, change: 0, icon: 'bi bi-cash-stack',        color: 'red',    progress: 0,  suffix: ''    },
  ];
  ...
  // ── Recent Taxpayers ──
  recentTaxpayers: Array<{ name: string; tin: string; type: string; date: string; status: string }> = [];

  // ── Recent Payments ──
  recentPayments: Array<{ ref: string; taxpayer: string; amount: number; type: string; date: string; status: string }> = [];
  ...
  // ── Quick Actions ──
  quickActions = [
    { label: 'New Taxpayer',    icon: 'bi bi-person-plus-fill',  route: '/taxpayers/create',  color: 'teal'   },
    { label: 'File VAT Return', icon: 'bi bi-arrow-repeat',      route: '/vat-returns/create', color: 'blue'  },
    { label: 'New Payment',     icon: 'bi bi-credit-card-fill',  route: '/payments/create',   color: 'green'  },
    { label: 'Issue Notice',    icon: 'bi bi-bell-fill',         route: '/notices/create',    color: 'orange' },
    { label: 'New Audit',       icon: 'bi bi-shield-fill-check', route: '/audits/create',     color: 'purple' },
    { label: 'View Reports',    icon: 'bi bi-bar-chart-fill',    route: '/reports',           color: 'navy'   },
  ];
  ...
  private loadDashboard(): void {
    ...
    this.dashboardService.loadAll()
      ...
      .subscribe({
        next: ([stats, taxpayers, payments, chartData, , , , , fiscalYears]) => {
  ...
  private applyFiscalYears(list: FiscalYear[]): void {
    ...
    this.currentFiscalYear = this.fiscalYears.find(y => y.isCurrentYear) ?? this.fiscalYears[0];
  ...
  private applyStats(stats: DashboardStats): void {
    ...
    this.complianceRate    = totalAll > 0 ? Math.round((totalFiled / totalAll) * 100) : 74;
    this.complianceFiled   = totalFiled;
    this.compliancePending = stats.pendingAudits ?? 0;

    // Normalised progress rings
    this.statCards[0].progress = Math.min(100, Math.round((stats.totalTaxpayers  / 30_000) * 100));
    this.statCards[1].progress = Math.min(100, Math.round((revenueInCr           / 600)    * 100));
    this.statCards[2].progress = Math.min(100, Math.round((stats.totalVatReturns / 20_000) * 100));
    this.statCards[3].progress = Math.min(100, Math.round((stats.totalPayments   / 40_000) * 100));
    this.statCards[4].progress = Math.min(100, Math.round((stats.pendingAudits   / 500)    * 100));
    this.statCards[5].progress = Math.min(100, Math.round((stats.pendingRefunds  / 200)    * 100));
  }
  ...
  private applyChartData(data: DashboardChartData): void {
    ...
    if (data?.paymentChart?.length) {
      ...
      this.payData   = data.paymentChart.map(d => +(d.value / 100_000).toFixed(1));
  ...
  drawGauge(): void {
    ...
    ctx.strokeStyle = '#eef2f8'; ctx.lineWidth = 20; ctx.lineCap = 'round'; ctx.stroke();
    const grad = ctx.createLinearGradient(cx - r, cy, cx + r, cy);
    grad.addColorStop(0, '#e74c3c'); grad.addColorStop(0.5, '#f59e0b'); grad.addColorStop(1, '#1faa8b');
    ctx.beginPath(); ctx.arc(cx, cy, r, start, val);
    ctx.strokeStyle = grad; ctx.lineWidth = 20; ctx.lineCap = 'round'; ctx.stroke();
    ctx.beginPath(); ctx.arc(cx, cy, r - 30, 0, 2 * Math.PI); ctx.fillStyle = '#fff'; ctx.fill();
    ctx.fillStyle = '#1a2340'; ctx.font = 'bold 28px Inter, sans-serif'; ctx.textAlign = 'center';
    ...
    ctx.textAlign = 'left';  ctx.fillText('0%',    cx - r - 10, cy + 20);
    ctx.textAlign = 'right'; ctx.fillText('100%', cx + r + 10, cy + 20);
  }
  ...
  drawVatChart(): void {
    ...
    for (let i = 0; i <= 4; i++) {
      ...
      ctx.strokeStyle = '#f0f4f8'; ctx.lineWidth = 1; ctx.stroke();
      ctx.fillStyle = '#aaa'; ctx.font = '10px Inter, sans-serif'; ctx.textAlign = 'right';
      ctx.fillText(`${Math.round(this.vatMax - (this.vatMax / 4) * i)}`, pad.left - 6, y + 4);
    }
  ...
  drawPayChart(): void {
    ...
    for (let i = 0; i <= 4; i++) {
      ...
      ctx.strokeStyle = '#f0f4f8'; ctx.lineWidth = 1; ctx.stroke();
      ctx.fillStyle = '#aaa'; ctx.font = '10px Inter, sans-serif'; ctx.textAlign = 'right';
      ctx.fillText(`${Math.round(this.payMax - (this.payMax / 4) * i)}`, pad.left - 6, y + 4);
    }
    this.payData.forEach((v, i) => {
      ...
      g.addColorStop(0, '#1a3f8f'); g.addColorStop(1, '#1a3f8f30');
      ctx.beginPath();
  ...
  drawDonut(): void {
    ...
    this.taxBreakdown.forEach(t => {
      ...
      ctx.beginPath(); ctx.moveTo(cx, cy); ctx.arc(cx, cy, 80, angle, angle + slice);
      ctx.closePath(); ctx.fillStyle = t.color; ctx.fill();
      angle += slice;
    });
    ctx.beginPath(); ctx.arc(cx, cy, 55, 0, 2 * Math.PI); ctx.fillStyle = '#fff'; ctx.fill();
    const totalCr = this.statCards[1].value;
    ctx.fillStyle = '#1a2340'; ctx.font = 'bold 20px Inter, sans-serif'; ctx.textAlign = 'center';
