...
export class AuditDetailComponent implements OnInit {
  ...
  onEdit(): void {
    if (this.auditCase?.id) {
      this.router.navigate(['../../edit', this.auditCase.id], {
  ...
  openDocRequestModal(): void {
    this.docForm.reset({ requestType: 'INITIAL' });
    this.showDocModal = true;
  }
  openFindingModal(): void {
    this.findingForm.reset({ findingType: 'INCOME_OMISSION', status: 'DRAFT' });
  ...
  submitQuery(): void {
    ...
    this.auditService
      .raiseQuery(this.auditCase.id, this.queryForm.value)
      .subscribe({
        next: (q) => {
          this.queries.push(q);
  ...
  submitDocRequest(): void {
    ...
    this.auditService
      .requestDocuments(this.auditCase.id, this.docForm.value)
      .subscribe({
        next: (d) => {
  ...
  submitFinding(): void {
    ...
    this.auditService
      .addFinding(this.auditCase.id, this.findingForm.value)
      .subscribe({
        next: (f) => {
  ...
  submitApproval(): void {
    ...
    this.auditService
      .approveAssessment(this.auditCase.id, this.approvalNotes)
      .subscribe({
        next: (a) => {
          this.assessment = a;
  ...
  calculateTaxFromFinding(): void {
    ...
    if (variance > 0) {
      this.findingForm.patchValue({
        additionalTax: +(variance * 0.25).toFixed(2),
      });
    }
  }
  ...
  canAddFinding(): boolean {
    return (
      ...
      [
        ...
        'FINDINGS_RECORDED',
        'DOCUMENT_REQUESTED',
      ].includes(this.auditCase.status)
    );
  }
  ...
  canApproveAssessment(): boolean {
    ...
    if (
      !['ASSESSMENT_PROPOSED', 'SUPERVISOR_REVIEW'].includes(
        this.auditCase.status,
      )
    )
      return false;
