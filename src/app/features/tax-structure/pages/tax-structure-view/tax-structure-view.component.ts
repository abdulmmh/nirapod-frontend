...
export class TaxStructureViewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.http
      .get<TaxStructure>(API_ENDPOINTS.TAX_STRUCTURES.GET(id))
      .subscribe({
        next: (data) => {
          this.tax = data;
        ...
        error: () => {
          ...
          this.isLoading = false;
          this.toast.error(
            'Failed to load tax structure details. Showing sample data.',
          );
        },
      });
  }
  ...
  onEdit(): void {
    this.router.navigate(['/tax-structure/edit', this.tax?.id]);
  }
  onBack(): void {
