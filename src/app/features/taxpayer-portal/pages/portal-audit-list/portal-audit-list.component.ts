...
@Component({
  ...
  styleUrls: ['./portal-audit-list.component.css'],
})
export class PortalAuditListComponent implements OnInit {
  ...
  view(id: number): void {
    this.router.navigate(['/my-portal/audits', id]);
  }

  requiresAction(c: AuditCase): boolean {
    return (
      ['DOCUMENT_REQUESTED', 'NOTICE_ISSUED', 'DEMAND_ISSUED'].includes(c.status) ||
      c.openQueryCount > 0
    );
  }
  ...
  getActionRequired(c: AuditCase): string {
    ...
    if (c.status === 'DEMAND_ISSUED')      return 'Pay or Appeal';
    return '';
  }
  ...
  getTypeClass(t: string): string {
    return { DESK:'type-desk', FIELD:'type-field', COMPREHENSIVE:'type-comp',
             VAT:'type-vat', REFUND:'type-refund', SPECIAL:'type-special' }[t] ?? '';
  }

  getTypeLabel(t: string): string {
    return { DESK:'Desk', FIELD:'Field', COMPREHENSIVE:'Comprehensive',
             VAT:'VAT', REFUND:'Refund', SPECIAL:'Special' }[t] ?? t;
  }
