...
export class PortalAppealCreateComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.demandNoticeId = params['demandNoticeId']
      ? +params['demandNoticeId']
      : null;
    this.assessmentNo = params['assessmentNo'] ?? null;
    this.caseNo = params['caseNo'] ?? null;
    this.demandedAmount = params['demandedAmount']
      ? +params['demandedAmount']
      : null;
  ...
  buildForm(): void {
    this.form = this.fb.group({
      ...
      reliefSought: [''],
      supportingEvidence: [''],
      disputedAmount: [this.demandedAmount ?? 0],
      remarks: [''],
    });
  }
  ...
  onSubmit(): void {
    ...
    this.appealService.fileAppeal(this.form.value).subscribe({
      ...
      error: (err) => {
        this.isSubmitting = false;
        this.submitError =
          err?.error?.message || 'Failed to file appeal. Please try again.';
      },
    });
  }
}
