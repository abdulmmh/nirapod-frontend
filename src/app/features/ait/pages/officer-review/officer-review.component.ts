...
import {
    AitDetailResponse, AitSourceType, AitStatus,
    AIT_STATUS_LABELS, AIT_STATUS_CLASSES, AIT_SOURCE_LABELS, AitDocument,
...
export class OfficerReviewComponent implements OnInit, OnDestroy {
    ...
    private statusOrder = [
        'DRAFT', 'SUBMITTED', 'PENDING', 'UNDER_REVIEW', 'APPROVED', 'CREDITED',
    ];
    ...
    loadRecord(id: number): void {
        this.isLoading = true;
        this.aitService.getById(id).pipe(takeUntil(this.destroy$)).subscribe({
            next: (data) => {
                this.ait = data;
                this.isLoading = false;
                this.reviewForm.patchValue({ approvedAmount: data.calculatedAitAmount });
    ...
    loadDocuments(aitId: number): void {
        this.http.get<AitDocument[]>(
            API_ENDPOINTS.AITS.DOCUMENTS.LIST(aitId)
        ).subscribe({
    ...
    onSubmitRecord(): void {
        ...
        // ✅ this.documents ব্যবহার করো — ait.documents নয়
        const payload = { attachmentIds: this.documents.map(d => d.id) };

        this.isActioning = true;
        this.aitService.submit(this.ait.id!, payload)
    ...
    deleteDoc(docId: number): void {
        ...
        this.http.delete(
            API_ENDPOINTS.AITS.DOCUMENTS.DELETE(this.ait.id, docId)
        ).pipe(takeUntil(this.destroy$)).subscribe({
            next: () => {
    ...
    downloadDoc(doc: AitDocument): void {
        if (!this.ait?.id) return;
        this.http.get(
            API_ENDPOINTS.AITS.DOCUMENTS.DOWNLOAD(this.ait.id, doc.id),
            { responseType: 'blob' }
        ).pipe(takeUntil(this.destroy$)).subscribe({
            next: blob => {
    ...
    onVerifyChallan(): void {
        ...
        this.isActioning = true;
        this.aitService.verifyChallan(this.ait.id!, this.challanForm.value)
            .pipe(takeUntil(this.destroy$)).subscribe({
                next: () => {
                ...
                error: (err) => {
                    this.toast.error(err?.error?.message ?? 'Challan verification failed.');
                    this.isActioning = false;
                },
            });
    }
    ...
    onAssign(): void {
        if (!this.ait) return;
        this.isActioning = true;
        this.aitService.assignToMe(this.ait.id!).pipe(takeUntil(this.destroy$)).subscribe({
            next: () => {
    ...
    onApprove(): void {
        ...
        this.aitService.approve(this.ait.id!, payload).pipe(takeUntil(this.destroy$)).subscribe({
            next: () => {
    ...
    onReject(): void {
        ...
        this.isActioning = true;
        this.aitService.reject(this.ait.id!, this.rejectForm.value)
            .pipe(takeUntil(this.destroy$)).subscribe({
                next: () => {
    ...
    onCredit(): void {
        if (!this.ait) return;
        this.isActioning = true;
        this.aitService.credit(this.ait.id!).pipe(takeUntil(this.destroy$)).subscribe({
            next: () => {
    ...
    canAct(): boolean {
        return !!this.ait && ['SUBMITTED','PENDING','UNDER_REVIEW','APPROVED']
            .includes(this.ait.status);
    }

    isStepDone(stepStatus: string): boolean {
        if (!this.ait || this.ait.status === 'REJECTED') return false;
        return this.statusOrder.indexOf(stepStatus) < this.statusOrder.indexOf(this.ait.status);
    }

    isStepActive(stepStatus: string): boolean { return this.ait?.status === stepStatus; }
    ...
    getSourceClass(s: AitSourceType): string {
        return ({ IMPORT:'cat-import', SUPPLIER:'cat-supplier', SALARY:'cat-salary',
                  CONTRACTOR:'cat-contractor', RENT:'cat-rent' } as any)[s] ?? '';
    }
    getSourceIcon(s: AitSourceType): string {
        return ({ IMPORT:'bi-box-seam', SUPPLIER:'bi-truck', SALARY:'bi-person-badge',
                  CONTRACTOR:'bi-tools', RENT:'bi-building' } as any)[s] ?? 'bi-receipt';
    }
}