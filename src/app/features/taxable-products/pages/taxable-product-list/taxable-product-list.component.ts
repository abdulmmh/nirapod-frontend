...
export class TaxableProductListComponent implements OnInit {
  ...
  constructor(private router: Router,
              private http: HttpClient,
               private taxableProductsService: TaxableProductsService) {}

  ngOnInit(): void {
    this.isLoading = true;
    this.taxableProductsService.getTaxableProducts().subscribe({
  ...
  delete(id: number): void {
    if (!confirm('Are you sure you want to delete this taxable product?')) return;
        this.taxableProductsService.deleteTaxableProduct(id).subscribe({
