...
export class PaymentViewComponent implements OnInit, OnDestroy {
  ...
  canApprove(): boolean {
    return this.payment?.status === 'Under Review' &&
           (this.authService.hasRole(Role.SUPERVISOR) ||
            this.authService.hasRole(Role.TAX_COMMISSIONER) ||
            this.authService.hasRole(Role.SUPER_ADMIN));
  }
  ...
  canReject(): boolean {
    return (this.payment?.status === 'Pending' || this.payment?.status === 'Under Review') &&
           (this.authService.hasRole(Role.TAX_OFFICER) ||
            this.authService.hasRole(Role.SUPERVISOR) ||
            this.authService.hasRole(Role.TAX_COMMISSIONER) ||
            this.authService.hasRole(Role.SUPER_ADMIN));
  ...
  hasAnyWorkflowAction(): boolean {
    return this.canMarkUnderReview() || this.canApprove() || this.canReject() || this.canCancel();
  }
  ...
  confirmAction(): void {
    ...
    if ((this.currentAction === 'Reject' || this.currentAction === 'Cancel') &&
    ...
    this.http.patch(API_ENDPOINTS.PAYMENTS.UPDATE_STATUS(this.payment.id), payload)
      .pipe(takeUntil(this.destroy$), finalize(() => (this.isActing = false)))
      .subscribe({
        next: () => {
          this.payment!.status = newStatus as any;
          if (this.actionRemarks.trim()) this.payment!.remarks = this.actionRemarks.trim();
  ...
  getModalConfig(): { title: string; sub: string; icon: string; headerClass: string; btnClass: string } {
    ...
    return map[this.currentAction] ?? { title: this.currentAction, sub: '', icon: 'bi bi-circle', headerClass: '', btnClass: '' };
  }
  ...
  onEdit(): void {
    if (this.payment?.id) {
      this.router.navigate(['edit', this.payment.id], { relativeTo: this.route });
    }
  }
