...
export class DocumentListComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.isLoading = true;
    this.http.get<Document[]>(API_ENDPOINTS.DOCUMENTS.LIST).subscribe({
      next: data => { this.documents = data;           this.isLoading = false; },
      error: ()   => { this.documents = this.fallback; this.isLoading = false; }
    });
  }
  ...
  delete(id: number): void {
    ...
    this.http.delete(API_ENDPOINTS.DOCUMENTS.DELETE(id)).subscribe({
      next: () => { this.documents = this.documents.filter(d => d.id !== id); },
      error: ()  => { this.documents = this.documents.filter(d => d.id !== id); }
    });
  }
